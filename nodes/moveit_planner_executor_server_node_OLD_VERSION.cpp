...
#include <nodes/moveit_planner_executor_server_node_OLD_VERSION.h>

// STATIC MEMBERS
const std::string MoveItPlannerExecutorServerNodeOldVersion::left_arm_planning_group = std::string("left_arm");
const std::string MoveItPlannerExecutorServerNodeOldVersion::right_arm_planning_group = std::string("right_arm");

// CONSTRUCTORS/DESTRUCTORS
MoveItPlannerExecutorServerNodeOldVersion::MoveItPlannerExecutorServerNodeOldVersion(const ros::NodeHandle& nh)
    : move_group_interface_larm_(left_arm_planning_group),
      move_group_interface_rarm_(right_arm_planning_group),
      robot_model_loader_("robot_description"), // look up robot description on ROS parameter server
      kinematic_model_(robot_model_loader_.getModel()) { // create kinematic model from loaded robot description
    ...
    std::cout << "[MoveIt Planner Executor Server Node - OLD VERSION] Constructed" << std::endl;
}

MoveItPlannerExecutorServerNodeOldVersion::~MoveItPlannerExecutorServerNodeOldVersion() {
    std::cout << "[MoveIt Planner Executor Server Node - OLD VERSION] Destroyed" << std::endl;
}

// CONNECTIONS
bool MoveItPlannerExecutorServerNodeOldVersion::initializeConnections() {
    if( volumetric_points_ ) {
        octomap_subscriber_ = nh_.subscribe(octomap_topic_, 1, &MoveItPlannerExecutorServerNodeOldVersion::octomapObstaclesCallback, this);
    }
    else {
        octomap_subscriber_ = nh_.subscribe(octomap_topic_, 1, &MoveItPlannerExecutorServerNodeOldVersion::octomapPointsCallback, this);
    }
...
// ADVERTISE SERVICES
void MoveItPlannerExecutorServerNodeOldVersion::advertiseServices() {
    plan_to_arm_goal_service_ = nh_.advertiseService("plan_to_arm_goal", &MoveItPlannerExecutorServerNodeOldVersion::planToArmGoalCallback, this);
    execute_to_arm_goal_service_ = nh_.advertiseService("execute_to_arm_goal", &MoveItPlannerExecutorServerNodeOldVersion::executeToArmGoalCallback, this);

    ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] Providing services for planning and executing MoveIt trajectories!");
    
    return;
}

// CALLBACKS
void MoveItPlannerExecutorServerNodeOldVersion::octomapPointsCallback(const sensor_msgs::PointCloud2& msg) {
    if( volumetric_points_ ) {
        // ignore message
        return;
    }
...
void MoveItPlannerExecutorServerNodeOldVersion::octomapObstaclesCallback(const visualization_msgs::MarkerArray& msg) {
    if( !volumetric_points_ ) {
        // ignore message
        return;
    }
...
// SERVICE CALLBACKS
bool MoveItPlannerExecutorServerNodeOldVersion::planToArmGoalCallback(val_moveit_planner_executor::PlanToArmGoal::Request&  req,
    ...
    if( req.planning_group_arm == req.LEFT_ARM ) {
        ...
        ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Invalid planning group arm %d requested; expect 0,1,2", req.planning_group_arm);
    ...
    if( req.collision_aware_planning ) {
        ...
        if( !updated_planning_scene ) {
            ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Could not update planning scene; ignoring request");
    ...
    // check success
    if( plan_success ) {
        ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] Found plan in %f seconds", moveit_plan.planning_time_);
    }
    else {
        ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Could not find plan");
...
bool MoveItPlannerExecutorServerNodeOldVersion::executeToArmGoalCallback(val_moveit_planner_executor::ExecuteToArmGoal::Request&  req,
    ...
    if( req.use_stored_robot_trajectory ) {
        ...
        if( plan_exists_ ) {
            ...
            ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Requested to execute stored robot trajectory, but no planned trajectory exists; ignoring request");
...
// GETTERS/SETTERS
double MoveItPlannerExecutorServerNodeOldVersion::getLoopRate() {
    return loop_rate_;
}

std::string MoveItPlannerExecutorServerNodeOldVersion::getTabString() {
    return std::string("\t");
}

// OCTOMAP HELPERS
bool MoveItPlannerExecutorServerNodeOldVersion::initializeOctomapVariables() {
...
void MoveItPlannerExecutorServerNodeOldVersion::markOctomapDataRecieved() {
    // update flag
...
bool MoveItPlannerExecutorServerNodeOldVersion::checkOctomapTimeout() {
...
// MOVEIT HELPERS
void MoveItPlannerExecutorServerNodeOldVersion::initializeMoveItVariables() {
    // robot model loader and kinematic model already created in class initializer list
    ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] KINEMATIC MODEL FRAME: %s", kinematic_model_->getModelFrame().c_str());

    // create kinematic state, which maintains configuration of the robot
    kinematic_state_ = moveit::core::RobotStatePtr(new moveit::core::RobotState(kinematic_model_));
    kinematic_state_->setToDefaultValues(); // TODO from planning scene?
    ...
    // planning scene interface; must be started before getting current state for joint groups
    planning_scene_interface_ = moveit::planning_interface::PlanningSceneInterface();

    // joint model group pointers for left and right arms; update from kinematic model
    joint_model_group_larm_ = kinematic_model_->getJointModelGroup(left_arm_planning_group);
    joint_model_group_rarm_ = kinematic_model_->getJointModelGroup(right_arm_planning_group);

    // joint model group pointers for left and right arms from MoveGroupInterface
    ...
    // print information
    ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] LEFT ARM MOVEIT INFO:");
    ...
    ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] RIGHT ARM MOVEIT INFO:");
...
bool MoveItPlannerExecutorServerNodeOldVersion::addPlanningSceneObstacles() {
    // check if octomap data is recent (function call implicitly checks if data is received)
    if( checkOctomapTimeout() ) {
        // old octomap data
        ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Octomap data more than %f seconds old; cannot update planning scene", octomap_timeout_);
        return false;
    }
...
bool MoveItPlannerExecutorServerNodeOldVersion::addPlanningSceneObstacles(sensor_msgs::PointCloud2& point_cloud) {
    ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Node requires octomap as visualization_msgs::MarkerArray, not sensor_msgs::PointCloud2 since volumetric points are required for collision-aware motion planning; please restart node with volumetric_points:=true");
    return false;
...
bool MoveItPlannerExecutorServerNodeOldVersion::addPlanningSceneObstacles(visualization_msgs::MarkerArray& obstacles) {
    // initialize vector of collision objects
    std::vector<moveit_msgs::CollisionObject> collision_objects;
    ...
    for( int i = 0 ; i < obstacles.markers.size() ; i++ ) {
        ...
        if( marker.type == marker.CUBE ) {
            ...
            ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Expected Octomap markers to be of type %d, but got %d instead", marker.CUBE, marker.type);
            return false;
        }
        ...
        if( marker.action == marker.ADD ) {
            ...
            ROS_WARN("[MoveIt Planner Executor Server Node - OLD VERSION] Expected Octomap markers to have action %d, but got %d instead", marker.ADD, marker.action);
            return false;
        }
...
void MoveItPlannerExecutorServerNodeOldVersion::clearPlanningSceneObstacles() {
...
int main(int argc, char **argv) {
    // initialize node
    ros::init(argc, argv, "MoveItPlannerExecutorServerNodeOldVersion");

    // initialize node handler
    ros::NodeHandle nh("~");

    // create node
    MoveItPlannerExecutorServerNodeOldVersion moveit_server_node(nh);
    ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] Node started!");
    ...
    ROS_INFO("[MoveIt Planner Executor Server Node - OLD VERSION] Node stopped, all done!");
